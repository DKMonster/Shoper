...
class Welcome extends CI_Controller {
	...
	function order() {
		...
		if($this->input->post('rule') == 'order') {
			...
			if($this->session->userdata('user_login_status')) {
				$dataOrderMain['buy_id'] = $this->session->userdata('user_id');
			}
			...
			if(sizeof($cart) == 0) {
				...
				if($dataOrderMain['buy_name'] != '' && $dataOrderMain['buy_phone'] != '' 
					...
					try {
						...
						$obj->Send['ReturnURL'] = base_url('complete/'.$dataOrderMain['id']); //付款完成通知回傳的網址
						$obj->Send['MerchantTradeNo'] = $dataOrderMain['id'];
						$obj->Send['MerchantTradeDate'] = date('Y/m/d H:i:s');
						$obj->Send['TotalAmount'] = $this->cart->total();
						$obj->Send['TradeDesc'] = '購買商品';
						$obj->Send['ChoosePayment'] = ECPay_PaymentMethod::Credit;

						// 訂購資料
						foreach ($dataOrderSub as $key => $value) {
							# code...
	...
	function complete() {
		...
		try {
			...
			$obj->HashIV = 'v77hoKGq4kWxNNIS';
			$obj->MerchantID = '2000132';
			$res = $obj->CheckOutFeedback();
			if($res['RtnCode'] == '1') {
				$id = $res['MerchantTradeNo'];
				if($this->mod_order->chk_exist($id)) {
					$dataArray = array('status'=> 1);
					$this->mod_order->update($id, $dataArray);
					...
					if($dataArray['password'] === $this->input->post('confirmPassword')) {
						...
						if(!$this->mod_member->get_once_by_email($dataArray['email'])) {
							...
							if($this->mod_member->insert($dataArray)) {
								...
								$this->send_email($this->input->post('email'), '歡迎加入Shoper！', 'Hi '.$dataArray['nickname'].'，感謝您註冊，有任何問題可以進入網站來詢問。');
	...
	function historys() {
		...
		if($this->mod_member->chk_login_status()) {
			...
			$view_data['order_list'] = $this->mod_order->get_user_order($this->session->userdata('user_id'));
	...
	function send_email($email, $subject, $content) {
		$this->email->from('service@shoper.com', 'Shoper Website');
		$this->email->to($email);
